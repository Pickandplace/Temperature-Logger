...
extern const rom char CONFIG_FILE_ID_START;
extern const rom char CONFIG_FILE_ID_END;
extern const rom char CONFIG_FILE_NAME;
//extern const rom char LOW_BATTERY_STR;
extern const  rom char LOG_NOT_STARTED;
extern rtccTimeDate TimeDate;
//extern BYTE TimeState;
    extern long periode;
    extern WORD LoggerID;
    extern unsigned char idString[7];
...
char astodec (unsigned long valeur);
//extern void FillDirectoryMSD(void);//Là
extern void DtASCII(char * tabASCII);
extern void FillDirectoryMSD(WORD LogFileSize);
...
//        	memcpypgm2ram((void *)&msd_buffer, (ROM void*)&inq_resp, sizeof(InquiryResponse));   //Inquiry response is 36 bytes total
    ...
    switch(MSDReadState)
    {
        ...
        case MSD_READ10_SECTOR:
    		...
    		if(LUNSectorRead(LBA.Val, (BYTE*)&msd_buffer[0]) != TRUE)
    		{
                ...
                {
                    //if( (msd_buffer[0]=='N') && (msd_buffer[1]=='O') && (msd_buffer[2]==' ')&&(msd_buffer[3]=='D'))
                    if(( (LBA.Val) == (FIRST_DATA_SECTOR+2))  )
                    { //If we have no logs in the EEPROM, we can indicate the batt level or that's the EEPROM is empty
                        //If we are reading the data.csv buffer, indicate the battery level or "NO DATA" if battery OK.
                       // tmp = msd_buffer[0]; //Works better
                        
                        if((BYTE)(msd_buffer[0]) == 0xFF)
                        {
                            if (LOW_BATT)
                            {
                                memset(msd_buffer,0x00,0x200);
                              //  strcpypgm2ram((char*)&msd_buffer,(const rom char*)&LOW_BATTERY_STR);
                            }
                            else
                            {
                                memset(msd_buffer,0x00,0x200);
                                strcpypgm2ram((char*)&msd_buffer,(const rom char*)&LOG_NOT_STARTED);
                            }
                        }
      ...
      /*      if(TIME_OK)
            {
                 if ( strstrrampgm(msd_buffer,(char rom*)"#THIS IS THE CONFIGURATION FILE") != 0x00 )
                 {//Here, we know the OS is trying to read the config file

                     //Check if it's the complete file
                     if ( strstrrampgm(msd_buffer,(char rom*)"#END OF THE CONFIGURATION FILE") != 0x00 )
                     {
                         //Yes, it's the config.txt file we're reading. Add time and date.

                       //  RtccReadTimeDate(&TimeDate);

                       // DtASCII(TimeDateASCIIi);

                         memcpypgm2ram((&msd_buffer+0x0C9),&STATUS_TIME_UPDATED,29);
...
{
    char heuredate[22];
    static char periodeASCII[8];
    WORD i;
    static unsigned long Temp;
    static char tempstr[5];
    static rtccTimeDate TimeDateOld;
    WORD_VAL LastUpdateTime, LastUpdateDate;
    static  WORD x;
extern BYTE_VAL STATUS_FLAG;
    extern BYTE StatePeriod;
    WORD LogFileSize;
    ...
    switch(MSDWriteState)
    {
        ...
        case MSD_WRITE10_SECTOR:
        {
            ...
            //which will contain the bCSWStatus letting it know an error occurred.
      		...
      		{
                    //Before we write any byte to the memory, check what is written:
                    //Allow writes into the config file
                    //Deny anything else by pretending the write was successfull.
                    //If we don't write the contains of the config.txt file, pretend everything
                    //Went OK, but don't write any data.
                  
           

                     if ( strstrrampgm(msd_buffer,(char rom*)"#THIS IS THE CONFIGURATION FILE") != 0x00 )
                     {//Here, we know the OS is trying to re-write the config file
                       
                         //Check if it's the complete file
                         if ( strstrrampgm(msd_buffer,(char rom*)"#END OF THE CONFIGU") != 0x00 )
                         {
                             WriteEnable = TRUE;
                             for(i=0x80;i<0x1D0;i++) //We don't want to take the start or end strings
                             {
                                 if (msd_buffer[i] == '#') //The period is entered like that: #xxxx (in seconds)
                                 {
                                     strncpy((char*)periodeASCII,(BYTE*)(&msd_buffer[i] +1),7);
                                     //Test if what we read is legit.
                                     for (i=0;i<7;i++)
                                     {
                                         if (periodeASCII[i] == 0x00)
                                             break;
                                         if(isdigit(periodeASCII[i]) == 0)
                                         {
                                             StatePeriod = ERROR_IN_PERIOD;
                                            break;
                                         }
                                     }

                                     periode = atol(periodeASCII);
                                     if(periode == 0)
                                        {
                                             StatePeriod = ERROR_IN_PERIOD;
                                            break;
                             ...
                             //Continue to search for cheat codes
                             for(i;i<0x1D0;i++)
                             {
                              if (msd_buffer[i] == '*') //id number is entered like that: *xxx
                                 {
                                     strncpy((char*)idString,(BYTE*)(&msd_buffer[i] +1),5);
                                     //Test if what we read is legit.
                                    /* for (i=0;i<7;i++)
                                     {
                                         if (idString[i] == 0x00)
                                             break;
                                         if(isdigit(idString[i]) == 0)
                                         {
                                             //StatePeriod = ERROR_IN_PERIOD;
                                            break;
                                         }
                         ...
                         //Write the file to the right place in EEPROM chip (0x2C000):
                          if(LUNSectorWrite(FIRST_DATA_SECTOR, (BYTE*)&msd_buffer[0], (LBA.Val==0)?TRUE:FALSE) != TRUE)
                            {
                            ...
                            break;
                             ...
                             } // end if ( strstrrampgm(msd_buffer,(char rom*)"#END OF THE CONFIGURATION FILE") != 0x00 )
                         else//not the end string
                         {
                        //Here, we have the start of the file but not the end. Stupid OS.  Return error.
                          if(MSDRetryAttempt < MSD_FAILED_WRITE_MAX_ATTEMPTS)
                            {
                                MSDRetryAttempt++;
                                break;
                     ...
                     }// end if ( strstrrampgm(msd_buffer,(char rom*)"#THIS IS THE CONFIGURATION FILE") != 0x00 )
                     else
                     {


                         // Take the date and time. Check also for values form config.txt
                     if ( strstrrampgm(msd_buffer,"LOGGER") )
                        {
                            //Here, we know the OS is writing the directory entry of the config file. Take the time and date

                         if( (msd_buffer[0x37] != 0x77) && (msd_buffer[0x38] != 0xDE) )

                         {
                            LastUpdateTime.v[0] = msd_buffer[0x36]; //Extract the time and date from the
                            LastUpdateTime.v[1] = msd_buffer[0x37]; //Root directory.
                            LastUpdateDate.v[0] = msd_buffer[0x38]; //The OS is writing it, should be updated with
                            LastUpdateDate.v[1] = msd_buffer[0x39]; //the computer time.

                            Temp = (LastUpdateTime.Val & 0b1111100000000000) >> 11; // The hours are coded on the left 5 bits. Mask and shift to get them aligned on the right side                     
                            TimeDate.f.hour =  astodec(Temp);

                            Temp =  (LastUpdateTime.Val & 0b0000011111100000) >> 5; //The minutes are coded on the b10-b5. Mask and shift to get them aligned on the right side
                            TimeDate.f.min =  astodec(Temp);

                            Temp  = (LastUpdateTime.Val & 0b0000000000011111);// The seconds are coded on the right side 5bits. Mask them.
                             TimeDate.f.sec =  astodec(Temp);

                            Temp  = ((LastUpdateDate.Val & 0b1111111000000000) >> 9) -0x14;// The years are coded on the left side  7 bits. Mask and shift them. Then, substract d20 (the PC years start in 1980, the PIC's in 2000.
                            TimeDate.f.year =  astodec(Temp);

                            Temp  = ((LastUpdateDate.Val & 0b0000000111100000) >> 5) ;// The month are coded on the middle 4 bits. Mask and shift them.
                            TimeDate.f.mon =  astodec(Temp);
                            
                            Temp  = ((LastUpdateDate.Val & 0b0000000000011111) );// The days are coded on the right side 5bits. Mask and shift them.
                            TimeDate.f.mday =  astodec(Temp);

                            //RtccReadTime(&TimeDateOld);
                            
                          //  if( (TimeDateOld.f.min != TimeDate.f.min) || (TimeDateOld.f.mday != TimeDate.f.mday))

                              if(!( (MAX_MIN < TimeDate.b[7]) || (MAX_SEC < TimeDate.b[6]) || (MAX_WDAY < TimeDate.b[5]) || (MAX_HOUR < TimeDate.b[4]) ||
                                    (MAX_MON < TimeDate.b[3]) || (MIN_MON > TimeDate.b[3]) || (MAX_MDAY < TimeDate.b[2]) || (MIN_MDAY > TimeDate.b[2]) ||
                                    (MAX_YEAR < TimeDate.b[0])))
                              {
                                  //Time correct
                                  TIME_READ_FROM_FILE = 1;
                                  //TIME_OK = 1;
                            ...
                            //Write the Directory to the right place in EEPROM chip, but only the original one:

                         //Actually, do not write anything..

                         WriteEnable = TRUE;
                         //LogFileSize = 0x200;
                         //FillDirectoryMSD(LogFileSize);
                               
                          //  for(x=0x80;x<0x200;x++)
                            //    msd_buffer[x]= 0x00;
                        /*  if(LUNSectorWrite(DIRECTORY_LOCATION, (BYTE*)&msd_buffer[0], (LBA.Val==0)?TRUE:FALSE)!= TRUE)
                            {
                            //The write operation failed for some reason.  Keep track of retry
                            //attempts and abort if repeated write attempts also fail.
                               if(MSDRetryAttempt < MSD_FAILED_WRITE_MAX_ATTEMPTS)
                      ...
                      //The OS is trying to write something else to the memory. Bad. Just pretend everything was OK.
                        //Writing success. Keep on going
                            //One LBA is written (unless an error occurred).  Advance state
                            //variables so we can eventually finish handling the CBW request.
                            LBA.Val++;
...
////
// Converts an ASCII char to its hex/decimal BCD value.
char astodec (unsigned long valeur)
{
    static char temp[5];
    ...
    {
        ultoa(valeur, temp);
        return( ( (temp[0] -0x30 )<<4) | (temp[1] - 0x30) );
    }

}

/*
    //Two cases: Or we have a number, or a letter. First, treat the numbers (bigger probability)
    if (valeur < 0x3A)
    {
        valeur = valeur - 0x30; //Convert from ASCII to Decimal
        return valeur;
    }
    else
    {
        //We're sure we have a letter. Shift it like the number and return it.
        valeur = valeur - 0x37; //Convert from ASCII to Decimal
        return (valeur);
    }
